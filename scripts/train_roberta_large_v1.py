...
MODEL_NAME = "klue/roberta-large"  # Updated model architecture
BATCH_SIZE = 8
EPOCHS = 3
...
TEST_SPLIT = 0.2
RANDOM_SEED = 2024
LOG_DIR = 'logs_roberta'
MODEL_DIR = 'models_roberta'
...
# Note: We are now fine-tuning the embedding model, so we do not freeze its parameters
...
class ClassificationModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        """
        Forward pass that computes embeddings using the CLS token and outputs logits.
        """
        ...
        return logits.squeeze(dim=1)  # Squeeze only the second dimension
