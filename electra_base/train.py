...
class Config:
    ...
    # Model parameters
    # Korean ELECTRA (well-tested and reliable for Korean NLP)
    ...
    MODEL_DIR = 'models_electra'
    CHECKPOINT_DIR = 'checkpoints_electra'
...
class ConversationAnomalyDetector(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        outputs = self.transformer(
            ...
            output_attentions=False  # We use our own attention weights
        )
        ...
        if self.use_attention_pooling:
            ...
            attention_weights_for_pooling = self.attention_dropout(attention_weights)

            # Weighted sum of hidden states
            pooled_output = torch.bmm(
                attention_weights_for_pooling.unsqueeze(1),  # (batch_size, 1, seq_len)
                sequence_output  # (batch_size, seq_len, hidden_size)
            ).squeeze(1)  # (batch_size, hidden_size)
        else:
            # Use CLS token
            pooled_output = sequence_output[:, 0]
            # For non-attention pooling, create uniform attention weights
            attention_weights = torch.ones_like(attention_mask).float() / attention_mask.sum(dim=1, keepdim=True)
        ...
        # Return logits and attention weights (before dropout, for supervision)
...
class Trainer:
    def __init__(
        ...
        self.global_step = 0
        self.attn_supervision_weight = 0.5  # Weight for attention supervision loss
    ...
    def train_epoch(self):
        ...
        for step, batch in enumerate(pbar):
            ...
            if mask_pos.any():
                ...
                loss = loss_win + self.attn_supervision_weight * attn_loss
            else:
                attn_loss = None
                loss = loss_win
