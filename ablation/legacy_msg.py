...
class Config:
    ...
    # Model parameters
    MODEL_NAME = "klue/roberta-small"  # RoBERTa for better context understanding
    BATCH_SIZE = 16
...
class Trainer:
    ...
    def train_epoch(self):
        ...
        for step, batch in enumerate(pbar):
            ...
            # 1) forward
            logits, all_attn_weights = self.model(input_ids, attention_mask)
            ...
            if mask_pos.any() and all_attn_weights is not None:
                # Use attention from the last layer (or average across layers)
                if isinstance(all_attn_weights, tuple):
                    # Get last layer attention and average across heads
                    attn_weights = all_attn_weights[-1]  # Last layer: [B, num_heads, seq_len, seq_len]
                    attn_weights = attn_weights.mean(dim=1)  # Average across heads: [B, seq_len, seq_len]
                    
                    # Get attention TO each position (average across FROM positions)
                    attn_weights = attn_weights.mean(dim=1)  # [B, seq_len]
                else:
                    # If it's already a processed tensor
                    attn_weights = all_attn_weights

                # Now gather attention at message positions
                # Expand msg_positions to match dimensions if needed
                msg_positions_expanded = msg_positions.unsqueeze(1)  # [B, 1, W]
                ...
                # Check if the model has attn_supervision_weight attribute
                if hasattr(self.model, 'attn_supervision_weight'):
                    loss = loss_win + self.model.attn_supervision_weight * attn_loss
                else:
                    loss = loss_win + 0.5 * attn_loss  # Default weight
            else:
                attn_loss = None
                loss = loss_win
