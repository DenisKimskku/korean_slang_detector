"""
Evaluate Gemini models on new_data.
Adapted from ablation/ablation_gemini.py for the new dataset.
"""
...
class Config:
    def __init__(self, model_name: str = "gemini-2.0-flash"):
        ...
        # Output paths
        self.LOG_DIR = f'/home/minseok/forensic/new_data_evaluation/logs_gemini_{model_name.replace("-", "_").replace(".", "_")}'
        self.RESULTS_DIR = f'/home/minseok/forensic/new_data_evaluation/results_gemini_{model_name.replace("-", "_").replace(".", "_")}'
...
class AdaptiveRateLimiter:
    """Manages API request timing with adaptive backoff"""

    def __init__(self, initial_requests_per_minute=30, min_requests_per_minute=10,
                 max_requests_per_minute=60, window_size=10):
...
# ------------------------- Gemini API Client ------------------------- #
class GeminiClient:
    def __init__(self, config_path: str, model_name: str, config: Config):
        self.model_name = model_name
...
# ------------------------- Evaluation Function ------------------------- #
def evaluate_with_gemini(gemini_client: GeminiClient, samples: List[Dict],
...
# ------------------------- Main Function ------------------------- #
def main():
    parser = argparse.ArgumentParser(description='Evaluate Gemini models on new dataset')
    ...
    logger.info(f"Starting Gemini evaluation with {args.model} on new dataset")
    ...
    try:
        ...
        metrics, detailed_predictions = evaluate_with_gemini(
            gemini_client, dataset.samples, config.BATCH_SIZE, logger, config
        )
        ...
        # Log results
        logger.info("\n" + "="*80)
        logger.info(f"GEMINI {args.model.upper()} EVALUATION RESULTS ON NEW DATA")
        logger.info("="*80)
        ...
        save_json(detailed_predictions, predictions_path)
