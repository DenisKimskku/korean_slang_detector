...
class Config:
    ...
    # Model parameters
    MODEL_NAME = "klue/roberta-large"  # RoBERTa for better context understanding
    BATCH_SIZE = 16
    ...
    # Output paths
    LOG_DIR = 'logs_pure_lm'
    MODEL_DIR = 'models_pure_lm'
    CHECKPOINT_DIR = 'checkpoints_pure_lm'
...
class SlidingWindowConversationDataset(Dataset):
    def __init__(
        ...
        stride: int = 5,
        max_length: int = 512,
        augmenter: Optional[ConversationAugmenter] = None,
    ...
    def load_data(self, input_dir: str):
        """Load conversations and create sliding windows"""
        ...
        for file in tqdm(json_files, desc="Loading conversations"):
            ...
            try:
                ...
                for conversation in data:
                    ...
                    if len(utterances) >= 3:  # Minimum conversation length
                        self._create_windows(utterances, conversation.get('id', 'unknown'))
                    
            except Exception as e:
                logger.error(f"Error loading {file}: {e}")
    
    def _create_windows(self, utterances: List[Dict], conv_id: str):
        """Create sliding windows from a conversation"""
        for start_idx in range(0, len(utterances) - self.window_size + 1, self.stride):
            end_idx = min(start_idx + self.window_size, len(utterances))
            
            window_messages = []
            window_labels = []
            
            for i in range(start_idx, end_idx):
                window_messages.append(utterances[i]['original_form'])
                window_labels.append(utterances[i].get('label', 0))
            
            # Apply augmentation if training
            if self.is_training and self.augmenter and any(window_labels):
                # Augment positive samples more frequently
                augment_prob = 0.7 if any(window_labels) else 0.2
                if random.random() < augment_prob:
                    window_messages, window_labels = self.augmenter.augment(
                        window_messages, window_labels
                    )
            
            # Create conversation text with special tokens
            conversation_parts = []
            for i, (msg, label) in enumerate(zip(window_messages, window_labels)):
                # Add speaker tokens for better context understanding
                speaker = f"[SPEAKER_{i % 2}]"  # Assume alternating speakers
                conversation_parts.append(f"{speaker} {msg}")
            
            conversation_text = " [SEP] ".join(conversation_parts)
            
            # Window label: 1 if any message is anomalous
            window_label = 1 if any(window_labels) else 0
            
            # Calculate anomaly positions for attention supervision
            anomaly_positions = [i for i, l in enumerate(window_labels) if l == 1]
    ...
    def _log_statistics(self):
        ...
        logger.info(f"  Total windows: {len(self.samples)}")
        logger.info(f"  Anomalous windows: {label_counts[1]} ({label_counts[1]/len(self.samples)*100:.2f}%)")
        logger.info(f"  Normal windows: {label_counts[0]} ({label_counts[0]/len(self.samples)*100:.2f}%)")
        
        # Analyze anomaly positions
        anomaly_positions_all = []
        for s in self.samples:
            if s['label'] == 1:
                anomaly_positions_all.extend(s['anomaly_positions'])
        
        if anomaly_positions_all:
            logger.info(f"  Average anomaly position in window: {np.mean(anomaly_positions_all):.2f}")
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'num_messages': torch.tensor(sample['num_messages'], dtype=torch.long)
        }
